...
namespace Naz.Hastane.Data.Entities {
    ...
    public class LabResultMap : ClassMap<LabResult> {
        ...
        public LabResultMap() {
			Table("LabResult");
			LazyLoad();
			CompositeId().KeyProperty(x => x.KNR).KeyProperty(x => x.SNR).KeyProperty(x => x.TANIM).KeyProperty(x => x.GRUP).KeyProperty(x => x.CODE).KeyProperty(x => x.TARIH).KeyProperty(x => x.IND);
